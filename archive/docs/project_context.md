...
## 🚨 LATEST STAKEHOLDER FEEDBACK (2026-01-19)
**From Mark Insley:**
1.  **Workflow Divergence:** "New Business" (Purchases) has a fundamentally different route/process than "Remortgage / Rate Swap" business.
2.  **Asana Process Review:** The "Mortgage Dynamic" board might NOT be the correct start point for all leads.
3.  **Redundancy:** Early columns in the Asana board may be duplicates of entry steps/decisions that happen *before* deciding if "Analysis" (getting quotes) is needed.
4.  **Usage Inconsistency:** Team members (Nwabisa, Dexter) are using Asana differently despite training.
5.  **Implication:** Hard-coding strict webhooks to the *current* Asana board structure carries risk as the board structure is being challenged/reviewed. **Focusing on the Base44 UI (The "Analysis" engine) is the safer, higher-value immediate step.**
6.  **Action Item (Lenders):** Reference lender data in Notion (even if scraped elsewhere) so other agents can access it.
7.  **Action Item (Docs):** "PL Bible" and guides must be added to Notion urgently (tell Colin). Critically important for agent context.
8.  **Action Item (Mortgage Magic):** Lead the onboarding call with Andrew alongside Nwabisa.
